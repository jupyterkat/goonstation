...
/datum/mapSwitchHandler
	...
	proc/setNextMap(trigger, mapName = "", mapID = "")
		...
		//tell jenkins, via opengoon, to compile with a new map
		...
		if (!data)
			throw EXCEPTION("No response from opengoon API route")

		if (data["error"])
			throw EXCEPTION("Received error from opengoon API: [data["error"]]")
