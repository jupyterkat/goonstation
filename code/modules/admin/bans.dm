...
/proc/editBan(data)
	...
	if (data["data_hub_callback"])
		...
		var/name = (isclient(adminC) && adminC.mob && adminC.mob.name ? adminC.mob.name : "N/A")
...
/proc/deleteBan(data)
	...
	if (data["data_hub_callback"])
		...
		var/key = (isclient(adminC) && adminC.key ? adminC.key : adminC)
		var/name = (expired ? "\[Expired\]" : "[isclient(adminC) && adminC.mob && adminC.mob.name ? adminC.mob.name : "N/A"]")
