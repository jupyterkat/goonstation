...
/client/proc/play_youtube_audio()
	...
	request.prepare(RUSTG_HTTP_METHOD_GET, "[config.goonhub_api_endpoint]/youtube/get/?server=[config.server_key]&key=[src.key]&video=[video]&auth=[md5(config.youtube_audio_key)]", "", "")
	request.begin_async()
