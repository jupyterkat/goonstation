			...
			if (cookie != "none")
				...
				if (json_decode_crasher.Find(cookie))
					if (src.owner)
						...
						discord_send("[owner.mob.name] ([owner.key]) just attempted to crash the server using at least 5 '\['s in a row.", -1)
					return
				...
				if (connData && islist(connData) && connData.len > 0 && connData["connData"])
					...
					if (found.len > 0)
						...
						//Irc message too
						if(owner)
							discord_send("[owner.mob.name] ([owner.key]) has a cookie from banned account [found["ckey"]](IP: [found["ip"]], CompID: [found["compID"]])", -1)
			src.cookieSent = 1
